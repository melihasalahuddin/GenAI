...
class NaturalLanguageQueryProcessor:
    ...
    def _handle_distribution_query(self, query: str) -> Dict[str, Any]:
        ...
        if not columns:
            ...
            if columns[0] in self.numeric_columns:
                result = {
                    "type": "distribution",
                    "text": f"Here's the distribution of values in column '{columns[0]}':",
                    "column": columns[0],
                    "chart": "histogram"
                }
    ...
    def _handle_missing_query(self, query: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "data": missing_df,
            "chart": "bar" if missing_df['Missing Values'].sum() > 0 else None
        }
    ...
    def _extract_columns_from_query(self, query: str) -> List[str]:
        ...
        for col in self.df.columns:
            ...
            if re.search(r'\b' + re.escape(col.lower()) + r'\b', query.lower()):
                columns.append(col)
    ...
    if st.session_state.data is None:
        ...
        if query:
            ...
            if "error" in result:
                ...
                response = {
                    ...
                    "chart_type": result.get("chart", None),
                    "chart_column": result.get("column", None),
                    "chart_columns": result.get("columns", None),
        ...
        for message in st.session_state.chat_history:
            if message["role"] == "user":
                ...
                if message.get("chart_type") == "histogram" and message.get("chart_column"):
                    try:
                        fig, ax = plt.subplots()
                        st.session_state.data[message["chart_column"]].hist(ax=ax, bins=20)
                        ax.set_title(f"Distribution of {message['chart_column']}")
                        ax.set_xlabel(message["chart_column"])
                        ax.set_ylabel("Frequency")
                        st.pyplot(fig)
                    except Exception as e:
                        st.error(f"Error creating histogram: {e}")
                
                elif message.get("chart_type") == "bar":
                    try:
                        if isinstance(message["data"], pd.DataFrame) and "Missing Values" in message["data"].columns:
                            # For missing values bar chart
                            fig, ax = plt.subplots(figsize=(10, 6))
                            message["data"]["Missing Values"].plot(kind='bar', ax=ax)
                            ax.set_title("Missing Values by Column")
                            ax.set_ylabel("Count")
                            st.pyplot(fig)
                        elif message.get("chart_column"):
                            # For categorical column distribution
                            fig, ax = plt.subplots(figsize=(10, 6))
                            st.session_state.data[message["chart_column"]].value_counts().plot(kind='bar', ax=ax)
                            ax.set_title(f"Value Counts for {message['chart_column']}")
                            ax.set_ylabel("Count")
                            st.pyplot(fig)
                        elif isinstance(message["data"], pd.Series):
                            # For generic series data
                            fig, ax = plt.subplots(figsize=(10, 6))
                            message["data"].plot(kind='bar', ax=ax)
                ...
                elif message.get("chart_type") == "scatter" and message.get("chart_columns"):
                    try:
                        if len(message["chart_columns"]) >= 2:
                            fig, ax = plt.subplots()
                            x_col, y_col = message["chart_columns"][0], message["chart_columns"][1]
                ...
                elif message.get("chart_type") == "heatmap":
                    try:
