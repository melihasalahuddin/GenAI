...
@st.cache_data
def generate_document_summary(question, api_key):
    try:
        # Set up LLM directly for summary to avoid circular dependency
        os.environ["GOOGLE_API_KEY"] = api_key
        ...
        combined_text = ""
        for i, doc in enumerate(st.session_state.docs[:5]):  # Use first 5 chunks
                    ...
                    with st.spinner("Setting up RAG pipeline..."):
                        ...
                        if rag_chain:
                            ...
                            with st.spinner("Generating document summary..."):
                                ...
                                summary, summary_error = generate_document_summary(summary_question, api_key)
...
if st.session_state.processing_complete:
    if st.button("Process a new document"):
        ...
        for key in list(st.session_state.keys()):
            if key != "api_key_set":  # Keep API key setting
                del st.session_state[key]
        
        # Reinitialize necessary session states
        st.session_state.docs = None
        st.session_state.rag_chain = None
        st.session_state.document_summary = None
        st.session_state.processing_complete = False
